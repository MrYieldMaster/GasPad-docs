...
:::note
If you are planning on developing on Butane locally and you haven’t already set up your own local node, refer to [the quickstart tutorial](./../../protocol/Evmos-cli/single-node), or follow the instructions in the [GitHub repository](https://github.com/BUTANE-Smart-Chain).
:::

:::tip
If you need to add Butane Testnet and Mainnet to Metamask wallet, head over [here](https://chainlist.org/?testnets=true&search=Butane) to add configurations.
:::
...
![metamask networks settings](/img/Group 1.png)
...
3. Click the `Connect Wallet` button under `Butane Testnet`
...
```bash
Butaned keys unsafe-export-eth-key dev0
```
...
## Registering for Butane Domains

Butane Domains are modelled after the Butane Name Service (BNS), link [here](https://bns.domains/), Butane users can register for an Butane Domains with the help of [this](https://Butane.domains/) service. This service will use Metamask to connect and allow users to search for a name not taken. The registration process will take some Butane token.
