...
After connecting the Ledger wallet to Metamask and connecting to the webpage, it will display our wallet formatted on `bech32` and `hex` representation, we need these values to make sure that the message that we are going to sign is the correct one.
...
This step should be done automatically by the same service that generated the message, in this case, will broadcast the transaction for you.
