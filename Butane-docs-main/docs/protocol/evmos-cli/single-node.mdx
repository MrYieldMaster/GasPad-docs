...
The simplest way to start a local Butane node is by using the provided helper script on the base level of the [Butane repository](https://github.com/BUTANE-Smart-Chain),
...
:::tip
To avoid overwriting any data for a real node used in production, it was decided to store the automatically generated testing configuration at `~/.tmp-bbcsd` instead of the default `~/.bbcsd`.
:::

When working with the `local_node.sh` script, it is necessary to extend all `bbcsd` commands, that target the local test node, with the `--home ~/.tmp-bbcsd` flag. This is mandatory, because the `home` directory cannot be stored in the `bbcsd` configuration, which can be seen in the output below. For ease of use, it might be sensible to export this directory path as an environment variable:

```
$ export TMP=$HOME/.tmp-evmosd`
$ bbcsd config --home $TMP
{
"chain-id": "BBC_9892272",
...
LOGLEVEL="info"
# Set dedicated home directory for the bbcsd instance
HOMEDIR="$HOME/.tmp-bbcsd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
bbcsd init $MONIKER --chain-id=$CHAINID
```
...
[configuration files](./configuration#client-configuration) are in `~/.bbcsd` by default, but you can overwrite the
...
```bash
bbcsd keys add my_validator
```

Once you have created a local account, go ahead and grant it some `abbcs` tokens in your chain's genesis file. Doing
so will also make sure your chain is aware of this account's existence:

```bash
bbcsd add-genesis-account my_validator 10000000000abbcs
```
...
bbcsd add-genesis-account my_validator 1000000000stake,10000000000abbcs
```
...
is provided, it defaults to the local node pubkey created via the `bbcsd init` command above.
...
bbcsd collect-gentxs
```

This command will add all the `gentxs` stored in `~/.bbcd/config/gentx` to the genesis file.
...
To run a node with the same key every time: replace `bbcd keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | bbcd keys add $KEY --recover
```

:::tip
bbc currently only supports 24 word mnemonics.
:::
...
To export your bbc key as an Ethereum private key (for use with [Metamask](./../../../use/connect-your-wallet/metamask)
for example):

```bash
bbcd keys unsafe-export-eth-key $KEY
```
...
butane supports using a file or OS keyring backend for key storage. To create and use a file
stored key instead of defaulting to the OS keyring, add the flag `--keyring-backend file` to any
relevant command and the password prompt will occur through the command line. This can also be saved
as a CLI config option with:

```bash
bbcd config keyring-backend file
```
...
:::danger
If you are running a **validator node**, always be careful when doing `bbcd unsafe-reset-all`. You should never use
...
```bash
rm $HOME/.bbcd/config/addrbook.json $HOME/.bbcd/config/genesis.json
bbcd tendermint unsafe-reset-all --home $HOME/.bbcd
```
