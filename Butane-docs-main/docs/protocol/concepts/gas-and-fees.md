...
Users need to pay a fee to submit transactions on the Butane network.
As fees are handled differently on Ethereum and Butane,
it is important to understand how the Butane blockchain implements an Ethereum-type fee calculation,
that is compatible with the Butane SDK.
...
Also, note the fees that are paid for interacting with smart contracts on Butane
can earn smart contract deployers a revenue. For information on this,
head to [develop](./../../develop/mainnet#revenue).

## Prerequisite Readings

- [Butane SDK Gas](https://github.com/BUTANE-Smart-Chain)
...
such as a Butane transaction that delegates to a dozen validators,
require more gas than simple, single-step transactions,
such as a Butane transaction to send tokens to another address.
...
### How are Fees Handled on butane?

Gas fees on butane are relatively straightforward. As a user, you specify two fields:
...
Validators for butane SDK-based chains can specify their `min-gas-prices` that they will enforce when selecting
transactions to include in blocks. Thus, transactions with insufficient fees will encounter delays or fail outright.

At the beginning of each block, fees from the previous block are after which they can
...
Originally, a user would specify a `GasPrice` and `GasLimit` within a transaction—much like a Butane SDK transaction. A
...
### How are Gas and Fees Handled on Butane?

Fundamentally, Butane is a Butane SDK chain that enables EVM compatibility as part of a Butane SDK module. As a result of
 this architecture, all EVM transactions are ultimately encoded as Butane SDK transactions and update a Butane
 SDK-managed state.

Since all transactions are represented as Butane SDK transactions, transaction fees can be treated identically across
execution layers. In practice, dealing with fees includes standard Butane SDK logic, some Ethereum logic, and custom
Butane logic. For the most part, fees are collected by the `fee_collector` module, then paid out to validators and
delegators. A few key distinctions are as follows:

1. Fee Market Module

    In order to support EIP-1559 gas and fee calculation on Butane’ EVM layer, Butane tracks the gas supplied for each
    ...
    For EVM transactions, each node bypasses their local `min-gas-prices` configuration, and instead applies EIP-1559 fee
     ...
     surplus is considered a priority tip. This allows validators to compute Ethereum fees without applying Butane SDK
     fee logic.

    Unlike on Ethereum, the `BaseFee` on Butane is not burned, and instead is distributed to validators and delegators.
...
2. EVM Gas Refunds
    ...
    Butane refunds a fraction (at least 50% by default) of the unused gas for EVM transactions to approximate the current
    behavior on Ethereum. [Why not always 100%?](https://github.com/BUTANE-Smart-Chain)

3. Revenue Module

    Butane developed the Revenue Module as a way to reward developers for creating useful dApps—any contract that is
    registered with Butane’ Revenue Module rewards a fraction of the transaction fee (currently 95%) from each transaction
...
2. Nodes receive transactions for a subsequent block and gossip these transactions to peers
    * These can be sorted and prioritized by the included fee price (using EIP-1559 fee priority mechanics for EVM
    transactions - [code snippet](https://github.com/BUTANE-Smart-Chain)),
     to be included in the next block
3. Nodes run `BeginBlock` for the subsequent block
    * The FeeMarket module calculates the `BaseFee` ([code snippet](https://github.com/Butane/ethermint/blob/89fdd1984826ea524cb9b8feb089a99b6cfe8ace/x/feemarket/keeper/abci.go#L14))
     to be applied for this block using the total `GasWanted` from the previous block.
    * The Distribution module [distributes](https://docs.Butane.network/main/modules/distribution#begin-block) the
...
4. For each valid transaction that will be included in this block, nodes perform the following:
    ...
    * Then, for standard Butane Transactions, nodes:
...
In the Butane SDK, gas is tracked in the main `GasMeter` and the `BlockGasMeter`:
...
(unlike Ethereum's `uint256` values, Butane SDK numericals are represented using [Big.Int](https://pkg.go.dev/math/big#Int)
 types, which are dynamically sized).

More information regarding gas as part of the Butane SDK can be found [here](https://docs.Butane.network/main/basics/gas-fees.html).

### Matching EVM Gas consumption

Butane is an EVM-compatible chain that supports Ethereum Web3 tooling. For this reason, gas consumption must be equatable
 with other EVMs, most importantly Ethereum.

The main difference between EVM and Butane state transitions, is that the EVM uses a [gas table](https://github.com/ethereum/go-ethereum/blob/master/params/protocol_params.go)
 for each OPCODE, whereas Butane uses a `GasConfig` that charges gas for each CRUD operation by setting a flat and
 per-byte cost for accessing the database.

+++ https://github.com/BUTANE-Smart-Chain
...
+++ https://github.com/Butane/ethermint/blob/098da6d0cc0e0c4cefbddf632df1057383973e4a/x/evm/keeper/state_transition.go#L188
...
In Butane, a minimum gas price is not enforced by the `AnteHandler` as the `min-gas-prices` is
...
For this same reason, in Butane it is possible to send transactions with `0` fees for transaction
...
(i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by Butane.

### Gas Estimation

Ethereum provides a JSON-RPC endpoint `eth_estimateGas` to help users set up a correct gas limit in their transactions.

For that reason, a specific query API `EstimateGas` is implemented in Butane. It will apply the transaction against the
...
Let’s say a user transfers tokens from Chain A to Butane via IBC-transfer and wants to execute an Butane transaction—however,
 they don’t have any Butane tokens to cover fees. The Butane SDK introduced `Tips` as a solution to this issue; a user can
...
To cover transaction fees using a tip, this user can sign a transaction with a tip and no fees, then send the transaction
 to a fee relayer. The fee relayer will then cover the fee in the native currency (Butane in this case), and receive the
  tip in payment, behaving as an intermediary exchange.

More on Butane Tips:

- [Butane Tips Docs](https://github.com/BUTANE-Smart-Chain)

## Dealing with gas and fees with the Butane CLI

When broadcasting a transaction using the Butane CLI client, users should keep into consideration the options available.
There are three flags to consider when sending a transaction to the network:

- `--fees`: fees to pay along with transaction; eg: 10aButane. Defaults to the required fees.
- `--gas`: the gas limit to set per-transaction; the default value is 200000.
- `--gas-prices`: gas prices to determine the transaction fee (e.g. 10aButane).
