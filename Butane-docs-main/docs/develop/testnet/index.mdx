...
To add Butane to Metamask, head over [here](https://chainlist.org/?testnets=true&search=Butane) to add Testnet and Mainnet.
:::

The Butane Testnet is a decentralized blockchain network
...
It is run by Validators using the same software as on the Butane Mainnet,
...
Using the Butane Testnet provides several benefits for dApp developers.
...
Secondly, the Butane Testnet provides a community-driven
...
The Butane Testnet [Faucet](https://faucet.Butane.dev/) distributes small amounts
...
clicking on the Wallet extension or by using the 
